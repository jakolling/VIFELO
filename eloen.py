# app_en_nt.py — English UI, Clubs & National Teams via ClubElo + eloratings.net
...
# Known aliases for national team names that often vary in the listing
TEAM_ALIASES = {
    # Common diacritics and language variations
...
def _name_matches(candidate: str, target: str) -> bool:
    """Case-insensitive match with basic normalization and alias support."""
    ...
    # Check alias lists either way
    for canon, alts in TEAM_ALIASES.items():
        canon_l = canon.lower()
        if t == canon_l and c in [a.lower() for a in alts + [canon]]:
            return True
        if c == canon_l and t in [a.lower() for a in alts + [canon]]:
            return True
        if c in [a.lower() for a in alts + [canon]] and t in [a.lower() for a in alts + [canon]]:
            return True
    return False

def _parse_year_snapshot(text: str, team_name: str):
    """
    Robustly parse a yearly snapshot for a team.
    Accepts formats like:
      '1. Spain. 2178'  or  '1. Spain 2178'  (extra spaces tolerated)
      '  40. Finland. 1763  '
    Returns the integer Elo if found, else None.
    """
    lines = text.splitlines()

    # Primary pattern: rank dot name (optional dot) rating
    # Example matches:
    #   "1. Spain. 2178"
    #   "1. Spain 2178"
    pat_generic = re.compile(r"^\s*\d+\.\s*(.*?)\s*(?:\.|\s)\s*([0-9]{3,4})\s*$", re.IGNORECASE)

    for ln in lines:
        s = ln.strip()
        m = pat_generic.match(s)
        if m:
            name = m.group(1).strip()
            elo_raw = m.group(2).strip()
    ...
    # Fallback: any line that contains the team name and ends with a 3–4 digit number
    tail_num = re.compile(r"([0-9]{3,4})\s*$")
    for ln in lines:
        s = ln.strip()
        if _name_matches(s, team_name) or team_name.lower() in s.lower():
            m2 = tail_num.search(s)
            if m2:
                try:
...
def fetch_national_history_yearly(team_name: str,
    ...
    Uses a tolerant parser and gentle throttling.
    """
    records = []
    ...
    for y in range(y0, y1 + 1):
        ...
        try:
            ...
            # Skip missing/unexpected pages silently
            pass
...
def _try_graph_endpoints(team_name: str) -> pd.DataFrame | None:
    """
    Try a few possible chart endpoints if any are publicly available.
    If nothing matches, return None (we'll fallback to yearly snapshots).
    """
    candidates = [
    ...
    for url in candidates:
        try:
            ...
            for ln in txt.splitlines():
                s = ln.strip()
                # Accept formats like: YYYY-MM-DD;VALUE  or  YYYY-MM-DD,VALUE
